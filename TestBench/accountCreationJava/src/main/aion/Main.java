...
public class Main {
    ...
    private static Address senderAcc, receiverAcc;
    private static Long tail_addr;
    private static String head_addr = "0x0a0000000000000000000000000000000000000000000";
    private static Integer queueMax;

    public static void main(String[] args) throws InterruptedException {

        String url = "tcp://127.0.0.1:8547";
        senderAcc = Address.wrap(args[0]);
        String defaultPassword = args[1];
        tail_addr = args[2].equals("0") ? Long.MAX_VALUE : Long.valueOf(args[2]);
        receiverAcc = Address.wrap(head_addr + tail_addr.toString());
        ...
        if (api.getWallet().unlockAccount(senderAcc, defaultPassword, 86400).getObject().equals(false)) {
            System.out.println("Unlock failed.");
            System.exit(0);
        }

        BigInteger bal = api.getChain().getBalance(senderAcc).getObject();
    ...
    private static int run(int totalTxs, int interval) throws InterruptedException {
        ...
        while (true) {
            try {
                ...
                queue.add(hash);
                tail_addr--;
                receiverAcc = Address.wrap(head_addr + String.valueOf(tail_addr));
