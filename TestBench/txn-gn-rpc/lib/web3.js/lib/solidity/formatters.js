...
var formatInputInt = function (value) {
    ...
    if(value.length > 32) // address
        var result = utils.padLeft(utils.toTwosComplement(value).toString(16), 64);
    else 
        var result = utils.padLeft(utils.toTwosComplement(value).toString(16), 32);
 ...
 * @returns {SolidityParam}
 */
const formatInputString = function (value) {
    let result = utils.fromUtf8(value).substr(2);
    let length = result.length / 2;
    let l = Math.floor((result.length + 31) / 32);
    let resBeforeLength = result.length;
    result = utils.padRight(result, l * 32);
    console.log('[formatInputString] l=' + l + ' result-before-len=' + resBeforeLength + ' result-after-len=' + result.length)
    return new SolidityParam(formatInputInt(length).value + result);
};
...
var formatOutputInt = function (param) {
    ...
    console.log('[formatOutputInt] ' + value)
...
var formatOutputUInt = function (param) {
    var value = param.staticPart() || "0";
    console.log('[formatOutputUInt] value ' + value + ' len ' + value.length)
    return new BigNumber(value, 16);
};
