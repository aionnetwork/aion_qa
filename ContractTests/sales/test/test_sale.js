...
DummyReceiver = artifacts.require('./DummyReceiver.sol');
SDT = artifacts.require('./SelfDestructTransfer.sol');
...
contract("Sale", (accs) => {
  it("should set the initial parameters correctly", async () => {
    const [_sale, _r1, _r2, _r3] = await Promise.all([Sale.new(), Receiver.new(), Receiver.new(), DummyReceiver.new()]);
  ...
  describe("#live()", () => {
    it("should be live after the first transaction (also check event)", async () => {
      ...
      assert.equal(found, 2, "both receivers should emit EndSale events");
    ...
    it("should not be live after the last transaction", async () => {
      ...
      assert.equal(found, 2, "all three receivers should emit EndSale events");
