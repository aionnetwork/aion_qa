// WARNING: This is meant to be used in a test environment
...
contract SavingsTest is Owned {
	...
	function withdraw(uint256 timestamp) notNullified returns(bool) {
		return withdrawTo(msg.sender, timestamp);
	}
	 ...
	 * the correct model.
	 */
	function withdrawTo(address addr, uint256 timestamp) postStart notNullified returns (bool) {
		uint _d = deposited[addr];
		uint _w = withdrawn[addr];

		uint diff = _withdrawTo(_d, _w, timestamp, total);
	...
	function bulkWithdraw(address[] addrs, uint256 timestamp) notNullified {
		for (uint i=0; i<addrs.length; i++)
			withdrawTo(addrs[i], timestamp);
	}
